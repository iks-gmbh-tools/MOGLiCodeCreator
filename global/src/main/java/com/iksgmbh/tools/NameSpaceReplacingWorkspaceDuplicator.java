...
/**
 * Helper, to rename namespaces of all Eclipse projects within the workspace,
 * e.g. from "de.iks.exampleA.frontend" in "com.iks.exampleB.client".
 *
 * For this purpose, the following three steps are performed: 1. Names of
 * directories in the file system are modified. 2. The 'package' instructions
 * within the java files are changed. 3. All occurrences files of the old
 * namespace (e.g. in configuration files or java comments) are replaced.
 *
 * Note: 1. Always, the whole workspace is modified. 2. Namespaces beginn always
 * with the highest level, e.g. 'de' or 'com'. 3. Namespaces end on the last
 * common part, that all project names in the workspace share. 4. In the file
 * system, each project contains a folder that equals the namespace. It must not
 * contain files. Only its subfolders can contain files. 5. Not existing target
 * folders will be created. 6. Existing target files will be deleted (see
 * initTarget).
 *
 * Difference to the NameSpaceReplacer: Source folders/projects remain unchanged
 * and have to removed manually !
 *
 * @author OberratR
 */
public class NameSpaceReplacingWorkspaceDuplicator {

    private static final String defaultSourceWorkspaceDir = "C:/dev/testeclipse/workspaces/PojoMemoryDb/PojoMemoryDb1";
    private static final String defaultTargetWorkspaceDir = "C:/dev/testeclipse/workspaces/PojoMemoryDb/PojoMemoryDb2";
    private static final String sourceNamespace = "com.iksgmbh.memorydb";
    private static final String targetNamespace = "com.iksgmbh.sql.pojomemodb";
    private static final String srcDir = "java";
    private static final String[] directoriesToIgnore = { "target" };
    private static final String[] filesToParse = { ".xml", ".txt", ".java", ".properties", ".mf", ".exsd", ".product", ".project" };
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            if (file.isDirectory() && ! file.getName().equals(".settings")) {
                ...
                if (file.getName().equals("pom.xml")) {
                    File outputFile = new File(targetWorkspaceDir, file.getName());
    ...
    private void replaceDirectory(File sourceDir, String targetDirPath) throws Exception {
        ...
        // System.out.println("Neues Verzeichnis: " +
        // targetDir.getAbsoluteFile());
        boolean ok = targetDir.mkdir();
        if (!ok) {
            throw new Exception("Verzeichnis konnte nicht erstellt werden: " + targetDir.getAbsolutePath());
        }
    ...
    private void replaceJavaDirectory(File sourceDir, String targetDirPath) throws Exception {
        ...
        if (! sourceDir.exists()) {
            throw new RuntimeException("Unkown source path: " + sourceDir.getAbsolutePath());
        }
        ...
        System.out.println("Package-Struktur im " + srcDir + "-Verzeichnis ge?ndert von <" + sourcePackageStructure
                + "> auf <" + targetPackageStructure + ">.");
        ...
        if (children != null) {            
            for (int i = 0; i < children.length; i++) {
                if (children[i].isDirectory())  {
                    ...
                    replaceFile(children[i], targetDir.getAbsolutePath());
                }
            }
        }
    }
    ...
    private String replaceName(String sourceName, boolean isDirectory) {
        ...
        if (pos > -1) {
            ...
            if (isDirectory) {
                System.out.println("");
                System.out
                        .println("Der Verzeichnisname <" + sourceName + "> wurde ersetzt durch <" + targetName + ">.");
            } else {
    ...
    private void replaceDirectoryContent(File sourceDir, File targetDir) throws Exception {
        File[] children = sourceDir.listFiles();
        if (children != null) {
            for (int i = 0; i < children.length; i++) {
    ...
    private void replaceFileContent(File sourceFile, File targetFile) throws Exception {
        ...
        if (numMatches > 0) {
            System.out.println(
                    "In der Datei " + targetFile.getAbsoluteFile() + " gab es " + numMatches + " Ersetzungen.");
        }
    ...
    private void initTarget(final String targetWorkspaceDir) throws Exception {
        ...
        if (!target.exists()) {
            ...
            if (!ok) {
                throw new Exception(
                        "Target existiert nicht und konnte nicht erzeugt werden: " + target.getAbsolutePath());
            }
        ...
        if (files != null) {
            for (int i = 0; i < files.length; i++) {
                if (files[i].isDirectory()) {
                    ...
                    if (!ok) {
                        throw new Exception("Datei konnte nicht geloescht werden: " + files[i].getAbsolutePath());
                    }
                }
            }
            dir.delete();

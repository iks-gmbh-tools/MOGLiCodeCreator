...
public class PluginDataUnpacker {
	...
	public static void doYourJob(final PluginPackedData defaultData, 
			                     ...
			                     final Logger pluginLogger) throws MOGLiPluginException2 {
	 ...
	 */
	private void unpack() throws MOGLiPluginException2 {
		...
		for (final String artefact : artefacts) {
			...
			if (filenames != null) { // handle directory 
				...
				if (fileContent == null) {
					throw new MOGLiPluginException2("Error unpacking embedded resource file from jar. " +
	...
	private void createTemplateFiles(final String subdirName, final String[] filenames) throws MOGLiPluginException2 {
	...
	private void createFile(final File dir, final String filename, final String fileContent) throws MOGLiPluginException2 {
	...
	private String readContentFromFile(final String pathToResource) throws MOGLiPluginException2 {
		try {
			return FileUtil.readTextResourceContentFromClassPath(defaultData.getClazz(), pathToResource);
		} catch (IOException e) {
			throw new MOGLiPluginException2("Error reading file: " + pathToResource, e);
		}
	}
